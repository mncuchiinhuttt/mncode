...
// SkillsMarketplace bundles system, user, and discoverable skills.
type SkillsMarketplace struct {
	SystemSkills    []MarketplaceSkill `json:"systemSkills"`
	UserSkills      []MarketplaceSkill `json:"userSkills"`
	AvailableSkills []MarketplaceSkill `json:"availableSkills"`
...
func GetMarketplace() (SkillsMarketplace, error) {
	...
	available := make([]MarketplaceSkill, 0, len(freeMarketplaceSkills))
	for _, definition := range freeMarketplaceSkills {
		source := definition.Source
		if source == "" {
			source = "MCP Market"
		}
		available = append(available, MarketplaceSkill{
	...
	return SkillsMarketplace{SystemSkills: system, UserSkills: user, AvailableSkills: available, SourceURL: mcpMarketSkillsURL}, nil
}

// InstallMarketplaceSkill downloads a catalog skill's SKILL.md into the user
// skills directory.
func InstallMarketplaceSkill(slug string) (MarketplaceSkill, error) {
	definition, ok := findMarketplaceSkill(slug)
	if !ok {
		return MarketplaceSkill{}, fmt.Errorf("marketplace skill not found: %s", slug)
	}
	...
	return MarketplaceSkill{ID: "user:" + safeSkillFolder(definition.Slug), Slug: definition.Slug, Name: definition.Name, Description: definition.Description, Category: definition.Category, Source: "Installed", Installed: true, Free: true, MarketplaceURL: definition.MarketURL}, nil
}

// DeleteInstalledSkill removes a user-installed skill by id; system skills are
// protected.
func DeleteInstalledSkill(id string) error {
...
func installedSkills() (system, user []MarketplaceSkill) {
	...
	for _, skill := range catalog.Skills {
		...
		item := MarketplaceSkill{ID: "system:" + skill.Name, Slug: strings.ToLower(skill.Name), Name: skill.Name, Description: skill.Description, Source: "System", System: !isUser, Installed: true, Free: true}
		if isUser {
